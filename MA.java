...
public class MA extends MADistAlg {
	public MA(int i, MADistAlg[] da, MAScreen s) {
		super(i, da, s);
		init();
		initialize();
	}
	...
	protected String constructRow2(int g) {
		...
		if (needToSendReply[g])
			s = s + " Reply to " + MAScreen.node[g] + ".	 ";
		else if (deferred[g])
			s = s + " Defer reply to " + MAScreen.node[g] + ".	 ";
		return s;
	}
