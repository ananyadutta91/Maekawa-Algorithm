...
public class MAScreen extends JFrame implements ActionListener, KeyListener {
  // Constructor called with an algorithm code (ma)
  //   the number of nodes and a flag if it is an application or not.
  MAScreen(String s, String num, MAWaitObject w) {
    waitObject = w;

    // Search for the algorithm code.
    MAList alg = new MAList(MAXNODES, DEFAULTNODES);
  ...
  private MALog  logfile;          // The logfile object.
  private MADistAlg[] da;              // A reference to each node.
  private int      numNodes;         // Number of nodes.
  private MAWaitObject waitObject;     // For signalling on Exit.
