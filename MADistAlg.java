...
public abstract class MADistAlg extends JPanel implements ActionListener {
  ...
  protected void send(int message, int to, int parm1, int parm2) {
    ((MADistAlg) da[to]).receive(message, parm1, parm2);
  }
  ...
  protected MADistAlg(int i, MADistAlg[] d, MAScreen s) {
    me      = i;
    number  = d.length;
    da      = d;
    screen  = s;
  ...
  protected int addressButtons(String l, String special1, String special2) {
    ...
    for (int i = 0; i < number; i++)
      if (i != me) addButton(p, code, MAScreen.node[i], n++);
  ...
  private JPanel initButtons(String label) {
    ...
    l.setForeground(MAScreen.LABELCOLOR);
    p.add(l);
    return p;
   }
  ...
  protected void initialize() {
    ...
    title1.setForeground(MAScreen.colors[me]);
    title2.setForeground(MAScreen.colors[me]);
    ...
    for (int i = 0; i < number; i++)
      if (i != me) {
        ...
        row1[i].setForeground(MAScreen.colors[i]);
        row2[i].setForeground(MAScreen.colors[i]);

		//  Changed the code to add borders around the other nodes
  ...
  protected int FindNode(String b) {
    int toNode = 0;
    while ((toNode < number) &&
      !( b.trim().equals(MAScreen.node[toNode].trim())))
      toNode++;
    return toNode;
  }
  ...
  private   MADistAlg[] da;          // References to the nodes
  protected MAScreen    screen;      // Screen object.
